...
public class LoginRequestDtoValidator implements Validator {
    ...
    public void validate(Object target, Errors errors) {
        ...
        if (!accountRepository.existsByNickname(loginRequestDto.getNickname())){
            errors.rejectValue("nickname","invalid.nickname",new Object[]{loginRequestDto.getNickname()},"존재하지 않는 네임입니다.");
        }else{
            ...
            if(!passwordEncoder.matches(loginRequestDto.getPassword(), account.getPassword())){
                errors.rejectValue("password", "invalid.password", new Object[]{loginRequestDto.getPassword()},"패스워드가 일치 하지 않습니다.");
            }
        }
    }
}
